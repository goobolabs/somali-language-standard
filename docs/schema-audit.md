...
`schema_version`. The initial schema version was `1.0.0`. The SLS-0100 source
audit expands the lexicon schema to `1.1.0` so reviewed non-plural readings and
unresolved loanword status can be represented without guesses. Repository
release versions, standard document versions, and schema versions remain
independent.
...
Every `magac` entry must record reviewed grammatical gender and a `plural`
value, as required by SLS-0003 G11-R1 and G11-R3. That value is either an
attested lexical form or JSON `null` where review establishes that no ordinary
plural applies to the recorded reading. `null` never means unchecked. Plural
forms remain lexical evidence and are never generated by this schema.

`is_loanword` accepts `true`, `false`, or JSON `null`. The last value records
unresolved status and must not be treated as non-loanword; a `true` value still
requires an evidence-backed origin. These nullable states were added in schema
version 1.1.0 after the SLS-0100 audit showed that a required string plural and
Boolean loanword default would force unsupported lexical claims.

Definitions require a numbered sense, English definition, and Somali gloss.
References to example sentences use permanent `sls:sent:NNNNNN` identifiers.
Those references resolve against the example-sentence contract and are checked
for dangling IDs by the repository validator.
