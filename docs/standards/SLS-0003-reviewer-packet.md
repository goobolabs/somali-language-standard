...
**Per-rule progress:** 36 of 42 rules have maintainer native-speaker verdicts:
...
G16-R1 through G16-R5 were approved on 2026-08-30 as MR-13 through MR-48.
...
| **G15-R5** | Interrogative `ma` and negative `ma` MUST NOT be doubled in one construction to express a negative yes-or-no question; the reviewed fused negative interrogative construction MUST be used. | Ma Axmed baan hilibka cunin? | \*Cali hadiyad ma ma keenin? | **OK — MR-45** |
...
| **G16-R3** | A constituent question MUST use a reviewed interrogative form and MUST keep that interrogative constituent adjacent to its associated focus marking where the construction requires adjacency. | Xaggee baa Axmed aaday? | \*Xaggee Axmed baa aaday? | **OK — MR-46** |
| **G16-R4** | The `-kee`/`-tee` interrogative series MUST select exactly the same gender class and consonant allomorph as the head noun's definite article. | Macallinkee baa yimid?<br>Naagtee baa timid?<br>magaalada → magaaladee<br>meesha → meeshee | \*Naagkee baa timid? | **OK — MR-47** |
| **G16-R5** | An indirect question with a reviewed `halka` or `goorta` construction MUST be integrated as a subordinate constituent rather than punctuated or ordered as an independent direct question. | Weyddii halkuu tegayo. | Weyddii: “Xaggee buu tegayaa?” | **OK — MR-48** |
